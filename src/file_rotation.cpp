...
std::string FileRotationUtils::get_current_log_path(const FileSinkConfig& config) {
    return config.log_directory + config.base_filename + config.file_extension;
}
    
std::string FileRotationUtils::get_rotated_log_path(const FileSinkConfig& config, int rotation_number) {
    return config.log_directory + config.base_filename + config.file_extension + "." + std::to_string(rotation_number);
}
...
size_t FileRotationUtils::calculate_total_disk_usage(const FileSinkConfig& config) {
    size_t total_size = 0;
...
std::vector<std::string> FileRotationUtils::get_rotated_files_sorted(const FileSinkConfig& config) {
